...
type Att = { id: number; filename: string };
...
function FileLine({ cardId, att }: { cardId: number; att: Att }) {
  return (
    ...
    <div className="space-y-2">
      ...
      {current ? (
        <div className="flex items-center justify-between gap-2 rounded border border-slate-200 px-3 py-2">
          <a
            href={`/api/attachment/${current.id}`}
            target="_blank"
            rel="noopener"
...
export function WeitereAttachments({
  ...
  return (
    <div className="space-y-2">
      ...
      {items.map((a) => (
        <FileLine key={a.id} cardId={cardId} att={a} />
      ))}
      <UploadForm
        cardId={cardId}
        kind="other"
